...
bool DumpProtobufMessage(Message &msg, std::ostream &output)
{
...
#include <time.h>
#include <stdlib.h>
std::ofstream getRandFileOutput(std::string prefix)
{
  time_t rawtime;
  time(&rawtime);
  struct tm* timeinfo = localtime(&rawtime);
  int _rand = rand() % 90 + 10; // 10~99

  std::stringstream ss;
  ss << prefix << "-" << tm->tm_hour << "-" << tm->tm_min << "-" << tm->tm_sec << "-" << _rand;

  std::ofstream ofs;
  ofs.open(ss.str().c_str(), std::ofstream::out | std::ofstream::app | std::ofstream::binary);
  return ofs;
}

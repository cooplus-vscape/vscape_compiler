...
CodeGenFunction::CodeGenFunction(CodeGenModule &cgm, bool suppressNewContext)
    : CodeGenTypeCache(cgm), CGM(cgm), Target(cgm.getTarget()),
      ...
      TerminateHandler(nullptr), TrapBB(nullptr), dumpVirtual(false) {  // chao: dumpVirtual
...
void CodeGenFunction::FinishFunction(SourceLocation EndLoc) {
  ...
  if(this->dumpVirtual)
  {
    const FunctionDecl* fd = cast<FunctionDecl>(this->CurFuncDecl);
    vfss << "\t</field_access_list>\n";
    vfss << "</vf>\n\n";
    vfss.flush();
    llvm::coop_outs_vf() << vfss.str();
    vfss.str("");
    llvm::coop_outs_vf().flush();
  }
  // chao: end
...
void CodeGenFunction::StartFunction(GlobalDecl GD,
  ...
  // chao: print override information
  if (getLangOpts().DumpVerbose && D && isa<CXXMethodDecl>(D))
  {
    const CXXMethodDecl* M = cast<CXXMethodDecl>(D);
    if(M->isVirtual()) //  M->size_overridden_methods())
    {
      this->dumpVirtual = true;

      vfss << "<vf>\n";
      
      vfss << "\t<LOC>";
      if(Loc.isValid())
        vfss << xml_encode(Loc.printToString(getContext().getSourceManager()));
      else
        vfss << "NULL";
      vfss << "</LOC>\n";

      vfss << "\t<func_name>\n";
      vfss << "\t\t<mangled>" << CGM.getMangledName(M).str() << "</mangled>\n";
      vfss << "\t\t<demangled>" << xml_encode(M->getQualifiedNameAsString()) << "</demangled>\n";
      vfss << "\t</func_name>\n";

      vfss << "\t<overridden_vf_list>\n";
      for(auto i = M->begin_overridden_methods(), e = M->end_overridden_methods(); i != e; i++)
      {
        vfss << "\t\t<func_name>\n";
        vfss << "\t\t\t<mangled>" << CGM.getMangledName(*i).str() << "</mangled>\n";
        vfss << "\t\t\t<demangled>" << xml_encode((*i)->getQualifiedNameAsString()) << "</demangled>\n";
        vfss << "\t\t</func_name>\n";
      }
      vfss << "\t</overridden_vf_list>\n";

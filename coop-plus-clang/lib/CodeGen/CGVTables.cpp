...
void CodeGenModule::EmitVTable(CXXRecordDecl *theClass) {
  // chao
  if(getLangOpts().DumpVerbose)
  {
    std::string str;
    llvm::raw_string_ostream Msg(str);
    Msg << "<class>\n";
    Msg << "\t<name>" << xml_encode(theClass->getQualifiedNameAsString()) << "</name>\n";
    Msg << "\t<layout>\n";
    std::string _tmp;
    llvm::raw_string_ostream tmp(_tmp);
    getContext().DumpRecordLayout(theClass, tmp);
    Msg << xml_encode(tmp.str());
    Msg << "\t</layout>\n";
    Msg << "</class>\n\n";
    llvm::coop_outs_cls() << Msg.str();
    llvm::coop_outs_cls().flush();
  }
  // chao (end)

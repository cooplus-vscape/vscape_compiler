...
llvm::Value *CodeGenFunction::EmitLoadOfScalar(LValue lvalue,
                                               SourceLocation Loc) {
  bool isFieldAccess = this->dumpVirtual && lvalue.isFieldAccess();
  if(isFieldAccess)
  {
    this->vfss << "\t\t<access>\n";
    this->vfss << "\t\t\t<type>" << "Read" << "</type>\n";
    this->vfss << "\t\t\t<depth>" << lvalue.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << lvalue.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitLoadOfScalar" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
  llvm::Value* V =  EmitLoadOfScalar(lvalue.getAddress(), lvalue.isVolatile(),
  ...
  if(isFieldAccess)
  {
    V->setFieldAccess(lvalue.getFieldOffset(), lvalue.getFieldAccessDepth());
  }
  return V;
}
...
void CodeGenFunction::EmitStoreOfScalar(llvm::Value *value, LValue lvalue,
                                        bool isInit) {
  if (this->dumpVirtual && lvalue.isFieldAccess())
  {
    this->vfss << "\t\t<access>\n";
    this->vfss << "\t\t\t<type>" << "Write" << "</type>\n";
    this->vfss << "\t\t\t<depth>" << lvalue.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << lvalue.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitStoreOfScalar" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
...
RValue CodeGenFunction::EmitLoadOfLValue(LValue LV, SourceLocation Loc) {
  if(this->dumpVirtual && LV.isFieldAccess())
  {
    this->vfss << "\t\t<access>\n";
    this->vfss << "\t\t\t<type>" << "Read" << "</type>\n";
    this->vfss << "\t\t\t<depth>" << LV.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << LV.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitLoadOfLValue" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
  if (LV.isObjCWeak()) {
...
RValue CodeGenFunction::EmitLoadOfBitfieldLValue(LValue LV) {
  bool isFieldAccess = this->dumpVirtual && LV.isFieldAccess();
  if(isFieldAccess)
  {
    this->vfss << "\t\t<access>\n";
    this->vfss << "\t\t\t<type>" << "Read" << "</type>\n";
    this->vfss << "\t\t\t<depth>" << LV.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << LV.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitLoadOfBitfieldLValue" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
  ...
  if(isFieldAccess)
  {
    Val->setFieldAccess(LV.getFieldOffset(), LV.getFieldAccessDepth());
  }
...
RValue CodeGenFunction::EmitLoadOfExtVectorElementLValue(LValue LV) {
  ...
  if(isFieldAccess)
  {
    ...
    this->vfss << "\t\t\t<depth>" << LV.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << LV.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitLoadOfExtVectorElementLValue" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
  ...
  if (!ExprVT) {
    ...
    Vec = Builder.CreateExtractElement(Vec, Elt);
    if(isFieldAccess)
    {
      Vec->setFieldAccess(LV.getFieldOffset(), LV.getFieldAccessDepth());
    }
...
RValue CodeGenFunction::EmitLoadOfGlobalRegLValue(LValue LV) {
  ...
  if(isFieldAccess)
  {
    ...
    this->vfss << "\t\t\t<depth>" << LV.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << LV.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitLoadOfGlobalRegLValue" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
  ...
  if(isFieldAccess)
  {
    Call->setFieldAccess(LV.getFieldOffset(), LV.getFieldAccessDepth());
  }
...
void CodeGenFunction::EmitStoreThroughLValue(RValue Src, LValue Dst,
                                             bool isInit) {
  if (this->dumpVirtual & Dst.isFieldAccess())
  {
    this->vfss << "\t\t<access>\n";
    this->vfss << "\t\t\t<type>" << "Write" << "</type>\n";
    this->vfss << "\t\t\t<depth>" << Dst.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << Dst.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitStoreThroughLValue" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
  if (!Dst.isSimple()) {
    if (Dst.isVectorElt()) {
...
void CodeGenFunction::EmitStoreThroughBitfieldLValue(RValue Src, LValue Dst,
  ...
  if (this->dumpVirtual & Dst.isFieldAccess())
  {
    ...
    this->vfss << "\t\t\t<depth>" << Dst.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << Dst.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitStoreThroughBitfieldLValue" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
...
void CodeGenFunction::EmitStoreThroughExtVectorComponentLValue(RValue Src,
  ...
  if (this->dumpVirtual & Dst.isFieldAccess())
  {
    ...
    this->vfss << "\t\t\t<depth>" << Dst.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << Dst.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitStoreThroughExtVectorComponentLValue" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
...
void CodeGenFunction::EmitStoreThroughGlobalRegLValue(RValue Src, LValue Dst) {
  if (this->dumpVirtual & Dst.isFieldAccess())
  {
    ...
    this->vfss << "\t\t\t<depth>" << Dst.getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << Dst.getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitStoreThroughGlobalRegLValue" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }
...
LValue CodeGenFunction::EmitUnaryOpLValue(const UnaryOperator *E) {
  ...
  case UO_Deref: {
    ...
    if(Addr.getPointer()->isFieldAccess())
    {
      unsigned depth = Addr.getPointer()->getFieldAccessDepth();
      int64_t offset = Addr.getPointer()->getFieldOffset();
      LV.setFieldAccess(offset, depth+1);
    }
...
static Address emitAddrOfFieldStorage(CodeGenFunction &CGF, Address base,
  ...
  {
    CGF.vfss << "\t\t<access>\n";
    CGF.vfss << "\t\t\t<type>" << "Access" << "</type>\n"; // someone will access it later.
    CGF.vfss << "\t\t\t<depth>" << "UNK" << "</depth>\n";;
    CGF.vfss << "\t\t\t<offset>" << offset.getQuantity() << "</offset>\n";
    CGF.vfss << "\t\t\t<clang>" << "static emitAddrOfFieldStorage" << "</clang>\n";
    CGF.vfss << "\t\t</access>\n";
  }
  *pOffsetInChars = offset.getQuantity();
...
LValue CodeGenFunction::EmitLValueForField(LValue base,
  ...
  if (field->isBitField()) {
    ...
    LValue LV = LValue::MakeBitfield(Addr, Info, fieldType, fieldAlignSource);
    LV.setFieldAccess(Info.StorageOffset.getQuantity());   // chao
    return LV;
  }
  ...
  if (rec->isUnion()) {
    ...
    // For structs, we GEP to the field that the record layout suggests.
    addr = emitAddrOfFieldStorage(*this, addr, field, &offsetInChars);
  ...
  if(offsetInChars >= 0)
    LV.setFieldAccess(offsetInChars);
...
CodeGenFunction::EmitLValueForFieldInitialization(LValue Base,
  ...
  int64_t offsetInChars = -1;
  Address V = emitAddrOfFieldStorage(*this, Base.getAddress(), Field, &offsetInChars);
  ...
  LValue LV = MakeAddrLValue(V, FieldType, FieldAlignSource);
  if(offsetInChars >= 0)
    LV.setFieldAccess(offsetInChars);
  return LV;
}
...
RValue CodeGenFunction::EmitCall(QualType CalleeType, llvm::Value *Callee,
  ...
  bool isFieldAccess = this->dumpVirtual && Callee->isFieldAccess();
  if(isFieldAccess)
  {
    this->vfss << "\t\t<access>\n";
    this->vfss << "\t\t\t<type>" << "Execute" << "</type>\n";
    this->vfss << "\t\t\t<depth>" << Callee->getFieldAccessDepth() << "</depth>\n";;
    this->vfss << "\t\t\t<offset>" << Callee->getFieldOffset() << "</offset>\n";
    this->vfss << "\t\t\t<clang>" << "CodeGenFunction::EmitCall" << "</clang>\n";
    this->vfss << "\t\t</access>\n";
  }

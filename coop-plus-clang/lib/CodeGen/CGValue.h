...
public:
  void setFieldAccess(int64_t offset, unsigned depth = 1) { // chao
    // Value* V is not NULL, right?
    V->setFieldAccess(offset, depth);
  }
  bool isFieldAccess() {
    return V->isFieldAccess();
  }
  unsigned getFieldAccessDepth() {
    return V->getFieldAccessDepth();
  }
  void increaseFieldAccessDepth() {
    V->increaseFieldAccessDepth();
  }
  int64_t getFieldOffset(){
    return V->getFieldOffset();
  }

...
llvm::Value *ItaniumCXXABI::getVirtualFunctionPointer(CodeGenFunction &CGF,
  ...
  // chao
  if(CGF.getLangOpts().DumpVerbose)
  {
    std::string str;
    ...
    Msg << "\t<LOC>";
    if(Loc.isValid())
      Msg << xml_encode(Loc.printToString(CGF.getContext().getSourceManager()));
    else
      Msg << "NULL";
    Msg << "</LOC>\n";
    ...
    Msg << "\t<caller_name>\n";
    FunctionDecl* caller = (FunctionDecl*)CGF.CurGD.getDecl();
    Msg << "\t\t<mangled>" << CGF.CGM.getMangledName(caller).str() << "</mangled>\n";
    Msg << "\t\t<demangled>" << xml_encode(caller->getQualifiedNameAsString()) << "</demangled>\n";
    //Msg << "\t\t<cls>" << xml_encode(caller->getParent()->getQualifiedNameAsString()) << "</cls>\n";
    Msg << "</caller_name>\n";

    Msg << "\t<callee_name>\n";
    Msg << "\t\t<mangled>" << CGF.CGM.getMangledName(MethodDecl).str() << "</mangled>\n";
    Msg << "\t\t<demangled>" << xml_encode(MethodDecl->getQualifiedNameAsString()) << "</demangled>\n";
    Msg << "\t\t<cls>" << xml_encode(MethodDecl->getParent()->getQualifiedNameAsString()) << "</cls>\n";
    Msg << "\t</callee_name>\n";

    Msg << "</callsite>\n\n";
    llvm::coop_outs_cs() << Msg.str();
    llvm::coop_outs_cs().flush();
  }
  // chao (end)

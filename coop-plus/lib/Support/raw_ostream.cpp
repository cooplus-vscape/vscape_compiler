...
#include <time.h>
#include <sstream>
const char* get_rand_string(const char* prefix){
  static time_t t = time(nullptr);
  static struct tm* _tm = localtime(&t);
	static int _rand = rand() % 100; // to avoid writing to same file, when multiple clang instances are running
  std::stringstream ss;
  ss << prefix << "-" << _tm->tm_hour << "-" << _tm->tm_min << "-" << _tm->tm_sec << "-" << _rand;
  return ss.str().c_str();
}

// chao
raw_ostream &llvm::coop_outs_cs(){
  std::error_code EC;
  static raw_fd_ostream S(get_rand_string("/tmp/coop.callsite.xml"), EC, sys::fs::F_Append);
  assert(!EC);
  return S;
}

raw_ostream &llvm::coop_outs_vf(){
  std::error_code EC;
  static raw_fd_ostream S(get_rand_string("/tmp/coop.vf.xml"), EC, sys::fs::F_Append);
  assert(!EC);
  return S;
}

raw_ostream &llvm::coop_outs_cls(){
  std::error_code EC;
  static raw_fd_ostream S(get_rand_string("/tmp/coop.cs.xml"), EC, sys::fs::F_Append);
  assert(!EC);
  return S;
}
